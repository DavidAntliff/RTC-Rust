...
use rust_rtc::patterns::{gradient_pattern};
...
fn main() {
    ...
    let mut floor_pattern = gradient_pattern(&RED, &WHITE);
    ...
    let mut middle_pattern = gradient_pattern(&BLUE, &WHITE);
    ...
    let mut right_pattern = gradient_pattern(&GREEN, &WHITE);
    ...
    let mut left_pattern = gradient_pattern(&YELLOW, &BLACK);
