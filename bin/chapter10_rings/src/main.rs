...
use rust_rtc::patterns::{ring_pattern};
...
fn main() {
    ...
    let mut floor_pattern = ring_pattern(&RED, &WHITE);
    ...
    wall.material.set_pattern(&ring_pattern(&BLACK, &RED));
    w.add_object(wall);
    ...
    let mut middle_pattern = ring_pattern(&BLUE, &WHITE);
    ...
    let mut right_pattern = ring_pattern(&GREEN, &BLACK);
    right_pattern.set_transform(&scaling(0.1, 0.1, 0.1)
    ...
    let mut left_pattern = ring_pattern(&YELLOW, &BLACK);
