...
fn main() {
    ...
    let floor_pattern_1 = gradient_pattern(&WHITE, &BLACK);
    let mut floor_pattern_2 = gradient_pattern(&RED, &GREEN);
    floor_pattern_2.set_transform(&scaling(0.5, 0.5, 0.5).then(&rotation_y(PI / 2.0)));
    let floor_pattern = stripe_pattern(&floor_pattern_1, &floor_pattern_2);
    ...
    let wall_pattern_1 = radial_gradient_pattern(&YELLOW, &BLACK, 0.0);
    //wall_pattern_1.set_transform(scaling(0.5, 0.5, 0.5));
    let wall_pattern_2 = radial_gradient_pattern(&BLACK, &YELLOW, 0.0);
    //wall_pattern_2.set_transform(scaling(0.5, 0.5, 0.5));
    let mut wall_pattern = ring_pattern(&wall_pattern_1, &wall_pattern_2);
    ...
    let middle_pattern_2 = stripe_pattern(&WHITE, &BLACK);
    let mut middle_pattern = ring_pattern(&BLUE, &middle_pattern_2);
    middle_pattern.set_transform(&scaling(0.2, 0.2, 0.2).then(&rotation_x(PI / 2.0)));
